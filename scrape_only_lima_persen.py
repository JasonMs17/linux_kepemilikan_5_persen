# 05/03/2026 ~ CATATAN BY JN
# LAST UPDATED: - 
# Script ini HANYA memproses pengumuman dengan "5%" kepemilikan saham
# Pengumuman lainnya akan di-skip dan tidak dimasukkan ke database
# Hal ini karena pengumuman sudah diambil lewat stockbitreports (MELALUI AWS)
...
lamp1_folder = Path(__file__).parent / "5_persen/pdf"
...
def process_lamp1_pdf(attachments):
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "extract_bounding_box.py", str(filepath)],
            cwd=str(Path(__file__).parent / "5_persen"),
            capture_output=True,
            text=True,
        )
...
while True:
    ...
    for item in replies:
        ...
        if key in existing_keys:
            ...
            # --- Check untuk "5%" di judul (HANYA PROSES YANG ADA 5%) ---
            if "5%" in judul:
                ...
                # Pengumuman tanpa "5%" - SKIP dan jangan masukkan ke database
                print(f"⏭ Pengumuman tidak mengandung '5%' - SKIP: {judul}")

        # Log opsional untuk semua pengumuman (commented out)
        # cleaned_item = {
        #     "pengumuman": {
...
print(f"\n=== SELESAI ===")
print(f"✅ Hanya pengumuman dengan '5%' yang dimasukkan ke database.")
print(f"⏭ Pengumuman lainnya di-skip dan tidak dimasukkan.")
